...
print("awesome!", file = log_file, flush=True)
...
def linear_evaluation(encoder, device, train_loader, test_loader, feature_dim, lr, num_epochs_eval):
    ...
    for epoch in range(num_epochs_eval):
        ...
        print(f'Epoch {epoch + 1}, Eval_Loss: {epoch_loss / len(train_loader):.4f}\n', file = log_file, flush=True)
...
for batch_size in batch_sizes:
    ...
    optimizer_vicreg = optim.Adam(vicreg_model.parameters(), lr=lr)
    vicreg_losses = []
    for epoch in range(num_epochs):
        ...
        vicreg_losses.append(min(epoch_loss / len(train_loader), 228))
    ...
    vicreg_acc = linear_evaluation(vicreg_model.encoder, device, train_loader, test_loader, 1024, lr, num_epochs_eval)
    ...
    optimizer_unbiased_vicreg = optim.Adam(unbiased_vicreg_model.parameters(), lr=lr)
    ...
    for epoch in range(num_epochs):
        ...
        unbiased_vicreg_losses.append(min(epoch_loss / len(train_loader), 228))
    ...
    unbiased_vicreg_acc = linear_evaluation(unbiased_vicreg_model.encoder, device, train_loader, test_loader, 1024, lr, num_epochs_eval)
    ...
    for epoch in range(num_epochs):        
        ...
        for (data, _) in progress_bar:
            ...
            epoch_loss += loss.item()
            progress_bar.set_postfix({'Loss': f'{loss.item():.4f}'})
        simclr_losses.append(min(epoch_loss / len(train_loader), 228))
    ...
    simclr_acc = linear_evaluation(simclr_model.encoder, device, train_loader, test_loader, 1024, lr, num_epochs_eval)
